...
import fr.acinq.eclair.wire.{FailureMessage, FailureMessageCodecs, LightningMessageCodecs}
...
object Sphinx extends Logging {
  ...
  def currentHopLength(payload: ByteVector): Int = {
    payload.head match {
      ...
      case _ =>
        ...
        val dataLength = LightningMessageCodecs.varIntCodec.decode(BitVector(payload.take(3))).require.value.toInt
  ...
  def parsePacket(privateKey: PrivateKey, associatedData: ByteVector, rawPacket: ByteVector): Try[ParsedPacket] = Try {
    ...
    ParsedPacket(payload, Packet(Version, nextPubKey, hmac, nextOnionPayload), sharedSecret)
  }
  ...
  def makePacket(sessionKey: PrivateKey, publicKeys: Seq[PublicKey], payloads: Seq[ByteVector], associatedData: ByteVector32): PacketAndSecrets = {
    ...
    val lastPacket = makeNextPacket(payloads.last, associatedData, ephemeralPublicKeys.last, sharedsecrets.last, EMPTY_PACKET, filler)
